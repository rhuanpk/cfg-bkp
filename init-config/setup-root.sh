...
set-personal-path() {
	default-action
	tee "$path_profile/$name_profile" <<- EOF
...
set-configurations() {
	...
	su "$user" -c "'$path_git/$name_cfgbkp/dunst/symlink-create.sh'"
	su "$user" -c "'$path_git/$name_cfgbkp/vimrc/symlink-create.sh'"
	"$path_git/$name_cfgbkp/rofi/symlink-create.sh" -r
	"$path_git/$name_cfgbkp/setload/configure.sh" -r
	"$path_git/$name_cfgbkp/swappiness/configure.sh" -r
	"$path_git/$name_linux/scripts/move2symlink.sh" -r
}

# Sets polkit policies.
set-polkit-rules() {
tee "$path_polkit/$name_polkit" << EOF
	...
	for program in "${!array_programs[@]}"; do
		...
		if [ "$(tr -dc ' ' <<< "${array_programs["$program"]}" | wc -m)" -lt 1 ]; then
			...
			if [ "$key_type" = 'asc' ]; then
				action-repeater wget -O - "$key_url" | gpg --dearmor -o "/etc/apt/trusted.gpg.d/$program.gpg"
			elif [ "$key_type" = 'gpg' ]; then
				action-repeater wget -O "/etc/apt/trusted.gpg.d/$program.gpg" "$key_url"
			fi
			tee "/etc/apt/sources.list.d/$program.list" <<< "$repo_line"
...
post-install() {
	...
	su "$user" -c 'gsettings set org.gnome.desktop.interface color-scheme prefer-dark'
...
[ "${uid:-`id -u`}" -ne 0 ] && {
	echo -e "\n${color_red}Run while logged in as root!$format_reset\n"
	exit 1
}
